...
{	
	int wybor;
	double bokFigury1, bokFigury2, bokFigury3, promienKola;
	...
	switch(wybor)
	{
		case 1:
			...
			scanf("%lf", &bokFigury1);
			printf("Pole: %lf\nObwód: %lf\n", poleKwadratu(bokFigury1), obwodKwadratu(bokFigury1));
		break;
		
		case 2:
			printf("Podaj boki prostokąta:\n");
			scanf("%lf %lf", &bokFigury1, &bokFigury2);
			printf("Pole: %lf\nObwód: %lf\n", poleProstokata(bokFigury1, bokFigury2), obwodProstokata(bokFigury1, bokFigury2));
		break;
		
		case 3:
			printf("Podaj boki trójkąta:\n");
			scanf("%lf %lf %lf", &bokFigury1, &bokFigury2, &bokFigury3);

			if(bokFigury1 + bokFigury2 > bokFigury3 && bokFigury1 + bokFigury3 > bokFigury2 && bokFigury2 + bokFigury3 > bokFigury1)
			{
				printf("Pole: %lf\nObwód: %lf\n", poleTrojkata(bokFigury1, bokFigury2, bokFigury3), obwodTrojkata(bokFigury1, bokFigury2, bokFigury3));
			}
			else
		...
		case 4:
			...
			scanf("%lf", &promienKola);
			printf("Pole:%lf\nObwód:%lf\n", poleKola(promienKola), obwodKola(promienKola));
		break;
